...
OS_RESULT os_mut_release (OS_ID mutex) {
  ...
  p_mlnk = os_runtask->p_mlnk;
  if (p_mlnk == p_MCB) {
    os_runtask->p_mlnk = p_MCB->p_mlnk;
  }
  else {
    while (p_mlnk) {
...
OS_RESULT os_mut_wait (OS_ID mutex, U16 timeout) {
  ...
  if (p_MCB->level == 0) {
    ...
    p_MCB->p_mlnk = os_runtask->p_mlnk;
    os_runtask->p_mlnk = p_MCB; 
    goto inc;
  }
...
inc:p_MCB->level++;
  ...
  if (p_MCB->owner->prio < os_runtask->prio) {
