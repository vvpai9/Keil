...
void os_resort_prio (P_TCB p_task) {
...
res:os_rmv_list (p_task);
    os_put_prio ((P_XCB)p_CB, p_task);
  }
}
...
void os_psq_enq (OS_ID entry, U32 arg)  {
  /* Insert post service request "entry" into ps-queue. */
  U32 idx;

  idx = _inc_qi (os_psq->size, &os_psq->count, &os_psq->first);
  if (idx < os_psq->size) {
