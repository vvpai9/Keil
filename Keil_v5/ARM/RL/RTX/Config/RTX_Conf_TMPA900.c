 ...
 *----------------------------------------------------------------------------
 *      Name:    RTX_CONFIG.C
 *      Purpose: Configuration of RTX Kernel for Toshiba TMPA900
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
#include <RTL.h>
#include <TMPA900.H>                    /* TMPA900 definitions               */
...
//   <i> Default: 48000000  (48MHz SMC_TIMxx clock)
#ifndef OS_CLOCK
 ...
 *---------------------------------------------------------------------------*/
 ...
 #define TIMx(reg)      Timer0##reg
 #define VICVECTADDRx   VICVECTADDR2
#elif (OS_TIMER == 1)                                   /* Timer 1          */
 #define OS_TID_        2                               /*  Timer ID        */
 #define TIMx(reg)      Timer1##reg
 #define VICVECTADDRx   VICVECTADDR2
#elif (OS_TIMER == 2)                                   /* Timer 2          */
 #define OS_TID_        3                               /*  Timer ID        */
 #define TIMx(reg)      Timer2##reg
 #define VICVECTADDRx   VICVECTADDR3
#elif (OS_TIMER == 3)                                   /* Timer 3          */
 #define OS_TID_        3                               /*  Timer ID        */
 #define TIMx(reg)      Timer3##reg
 #define VICVECTADDRx   VICVECTADDR3
#elif (OS_TIMER == 4)                                   /* Timer 4          */
 #define OS_TID_        4                               /*  Timer ID        */
 #define TIMx(reg)      Timer4##reg
 #define VICVECTADDRx   VICVECTADDR4
#elif (OS_TIMER == 5)                                   /* Timer 5          */
 #define OS_TID_        4                               /*  Timer ID        */
 #define TIMx(reg)      Timer5##reg
 #define VICVECTADDRx   VICVECTADDR4
#else
...
#define OS_TRV          ((U16)(((double)OS_CLOCK*(double)OS_TICK)/256E6)-1)
#define OS_TVAL         TIMx(Value)                     /*  Timer Value     */
#define OS_TOVF         (TIMx(RIS) & 1)                 /*  Overflow Flag   */
#define OS_TFIRQ()      VICSOFTINT      = OS_TIM_;      /*  Force Interrupt */
#define OS_TIACK()      TIMx(IntClr)    = 0;            /*  Interrupt Ack   */ \
                        VICSOFTINTCLEAR = OS_TIM_;                             \
                        VICADDRESS      = 0;
#define OS_TINIT()      TIMx(Load)      = OS_TRV;       /*  Initialization  */ \
                        TIMx(Control)   = 0xEA;                                \
                        VICVECTADDRx    = (U32)os_clock_interrupt;             \

#define OS_LOCK()       VICINTENCLEAR   = OS_TIM_;      /* Task Lock        */
