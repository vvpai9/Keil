 ...
 *----------------------------------------------------------------------------
 *      Name:    MCI_SAM9XE.c
 *      Purpose: Multimedia Card Interface Driver for Atmel AT91SAM9XE
 *      Rev.:    V4.20
 *----------------------------------------------------------------------------
...
static BOOL Init (void) {
  ...
  pPMC->PMC_PCER   = (1 << AT91C_ID_PIOA) | (1 << AT91C_ID_PIOC) |
                     (1 << AT91C_ID_MCI);
...
static U32 CheckMedia (void) {
  ...
  if (!(pPIOC->PIO_PDSR & AT91C_PIO_PC9)) {
