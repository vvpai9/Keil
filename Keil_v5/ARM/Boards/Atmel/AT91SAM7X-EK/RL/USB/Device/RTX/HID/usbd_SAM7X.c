 ...
 *----------------------------------------------------------------------------
 *      Name:    usbd_SAM7X.c
 *      Purpose: Hardware Layer module for Atmel AT91SAM7X
 *      Rev.:    V4.75
 *----------------------------------------------------------------------------
...
static int USB_GetSizeEP (U32 EPNum) {
  switch (EPNum & 0x0F) {
    ...
    case 4:
    case 5:
      return (256);                     /* Maximum size is 256 bytes          */
    default:
...
void USBD_Connect (BOOL con) {
  if (con) {
    pUDP->UDP_TXVC &= ~AT91C_UDP_TXVDIS;          /* Enable Transciever       */
  } else {
    pUDP->UDP_TXVC |=  AT91C_UDP_TXVDIS;          /* Disable Transciever      */
  }
}
...
U32 USBD_WriteEP (U32 EPNum, U8 *pData, U32 cnt) {
  ...
  if (pUDP->UDP_CSR[EPNum] & AT91C_UDP_TXPKTRDY)      /* If hw not rdy to send*/
    return (0);
