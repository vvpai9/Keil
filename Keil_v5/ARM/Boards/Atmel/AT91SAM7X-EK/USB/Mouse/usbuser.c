...
void USB_Reset_Event (void) {
  USB_ResetCore();
  LED_Off (LED_4 | LED_3);                  /* Turn Off LED4, LED3 */
}
#endif
...
void USB_Configure_Event (void) {
  ...
  if (USB_Configuration) {                  /* Check if USB is configured */
    GetMouseInReport();
    USB_WriteEP(HID_EP_IN, MouseInReport, sizeof(MouseInReport));
    LED_On (LED_3);                         /* Turn On LED3 */
  } else {
    LED_Off (LED_3);                        /* Turn Off LED3 */
  }
}
#endif
