...
SMC_BASE        EQU     0xFFFFE400      ; SMC                     Base Address
...
SDRAMC0_BASE    EQU     0xFFFFE200      ; SDRAMC0                Base Address
SDRAMC1_BASE    EQU     0xFFFFE800      ; SDRAMC1                Base Address
...
;// <h> SDRAM Controllers (SDRAMCs)

;//   <e> SDRAM0 Controller (SDRAMC0)
SDRAMC0_SETUP   EQU     1

;//     <h> Refresh Timer Register (SDRAMC0_TR)
;//       <o0.0..11>  COUNT: SDRAMC Refresh Timer Count <0-4095>
;//                   <i> This 12-bit field is loaded into a timer that generates
;//                   <i> the refresh pulse
;//     </h>
SDRAMC0_TR_Val  EQU     0x0000030D

;//     <h> Configuration Register (SDRAMC0_CR)
...
;//     </h>
SDRAMC0_CR_Val  EQU     0x85227259

;//     <h> Low Power Register (SDRAMC0_LPR)
...
;//       <o0.8..9>   TXSR: Temperature Compensated Self-refresh (only for low-power SDRAM) 
...
;//     </h>
SDRAMC0_LPR_Val EQU     0x00000000

;//     <h> Memory Device Register (SDRAMC0_MDR)
;//       <o0.0..1>     MD: Memory Device Type
;//                   <0=> SDRAM  <1=> Low-power SDRAM
;//     </h>
SDRAMC0_MDR_Val EQU     0x00000000

;//   </e> SDRAM0 Controller (SDRAMC0)

;//   <e> SDRAM1 Controller (SDRAMC1)
SDRAMC1_SETUP   EQU     0

;//     <h> Refresh Timer Register (SDRAMC1_TR)
;//       <o0.0..11>  COUNT: SDRAMC Refresh Timer Count <0-4095>
;//                   <i> This 12-bit field is loaded into a timer that generates
;//                   <i> the refresh pulse
;//     </h>
SDRAMC1_TR_Val  EQU     0x0000011F

;//     <h> Configuration Register (SDRAMC1_CR)
;//       <o0.0..1>   NC: Number of Column Bits
;//                   <0=> 8  <1=> 9  <2=> 10 <3=> 11
;//       <o0.2..3>   NR: Number of Row Bits
;//                   <0=> 11 <1=> 12 <2=> 13
...
;//                   <i> and an Activate Command, in cycles
;//     </h>
SDRAMC1_CR_Val  EQU     0x85227259

;//     <h> Low Power Register (SDRAMC1_LPR)
;//       <o0.0..1>   LPCB: Low-power Configuration Bits
;//                   <0=> Low-power feature inhibited
;//                   <1=> Self-refresh command issued after access
;//                   <2=> Power-down command issued after access
...
;//                   <2=> Low-power mode activated 128 clock cycles after last transfer
;//     </h>
SDRAMC1_LPR_Val EQU     0x00000000

;//     <h> Memory Device Register (SDRAMC1_MDR)
;//       <o0.0..1>     MD: Memory Device Type
;//                   <0=> SDRAM  <1=> Low-power SDRAM
;//     </h>
SDRAMC1_MDR_Val EQU     0x00000000
...
; Setup Bus Matrix (MATRIX) ----------------------------------------------------
                ...
                LDR     R2, =MATRIX_PRBS0_Val
                STR     R1, [R0, #MATRIX_PRAS0_OFS]
                STR     R2, [R0, #MATRIX_PRBS0_OFS]
                LDR     R1, =MATRIX_PRAS1_Val
                LDR     R2, =MATRIX_PRBS1_Val
                STR     R1, [R0, #MATRIX_PRAS1_OFS]
                STR     R2, [R0, #MATRIX_PRBS1_OFS]
                LDR     R1, =MATRIX_PRAS2_Val
                LDR     R2, =MATRIX_PRBS2_Val
                STR     R1, [R0, #MATRIX_PRAS2_OFS]
                STR     R2, [R0, #MATRIX_PRBS2_OFS]
                LDR     R1, =MATRIX_PRAS3_Val
                LDR     R2, =MATRIX_PRBS3_Val
                STR     R1, [R0, #MATRIX_PRAS3_OFS]
                STR     R2, [R0, #MATRIX_PRBS3_OFS]
                LDR     R1, =MATRIX_PRAS4_Val
                LDR     R2, =MATRIX_PRBS4_Val
                STR     R1, [R0, #MATRIX_PRAS4_OFS]
                STR     R2, [R0, #MATRIX_PRBS4_OFS]
                LDR     R1, =MATRIX_PRAS5_Val
                LDR     R2, =MATRIX_PRBS5_Val
                STR     R1, [R0, #MATRIX_PRAS5_OFS]
                STR     R2, [R0, #MATRIX_PRBS5_OFS]
                LDR     R1, =MATRIX_PRAS6_Val
                LDR     R2, =MATRIX_PRBS6_Val
                STR     R1, [R0, #MATRIX_PRAS6_OFS]
                STR     R2, [R0, #MATRIX_PRBS6_OFS]
                LDR     R1, =MATRIX_PRAS7_Val
                LDR     R2, =MATRIX_PRBS7_Val
                STR     R1, [R0, #MATRIX_PRAS7_OFS]
                STR     R2, [R0, #MATRIX_PRBS7_OFS]
                ENDIF   ; of IF      MATRIX_SETUP != 0


; Setup External Bus Interface (EBI) -------------------------------------------

                IF      (:LNOT::DEF:NO_EBI_INIT):LAND:(EBI0_SETUP != 0)
                LDR     R0, =MATRIX_BASE
                LDR     R1, =EBI0_CSA_Val
                STR     R1, [R0, #EBI0_CSA_OFS]
                ENDIF   ; of IF      (:LNOT::DEF:NO_EBI_INIT):LAND:(EBI0_SETUP != 0)

                IF      (:LNOT::DEF:NO_EBI_INIT):LAND:(EBI1_SETUP != 0)
                LDR     R0, =MATRIX_BASE
                LDR     R1, =EBI1_CSA_Val
                STR     R1, [R0, #EBI1_CSA_OFS]
                ENDIF   ; of IF      (:LNOT::DEF:NO_EBI_INIT):LAND:(EBI1_SETUP != 0)


; Setup SDRAM0 Controller (SDRAMC0) --------------------------------------------

                ; Setup SDRAM Controller if enabled
                IF      (:LNOT::DEF:NO_SDRAM_INIT):LAND:(SDRAMC0_SETUP != 0)
                ...
                LDR     R0, =SDRAMC0_BASE
                LDR     R2, =EBI0_CS1_ADDRESS
                MOV     R3, #0

                ; Setup Control Register of SDRAM Controller
                LDR     R1, =SDRAMC0_CR_Val
...
Wait_SDRAMC_1   SUBS    R1, R1, #1
                ...
                LDR     R1, =SDRAMC0_TR_Val
                STR     R1, [R0, #SDRAMC_TR_OFS]

                ; Setup Other SDRAM Controller Registers
                LDR     R1, =SDRAMC0_LPR_Val
                STR     R1, [R0, #SDRAMC_LPR_OFS]
                LDR     R1, =SDRAMC0_MDR_Val
                STR     R1, [R0, #SDRAMC_MDR_OFS]
                ENDIF   ; of IF      (:LNOT::DEF:NO_SDRAM_INIT):LAND:(SDRAMC0_SETUP != 0)
