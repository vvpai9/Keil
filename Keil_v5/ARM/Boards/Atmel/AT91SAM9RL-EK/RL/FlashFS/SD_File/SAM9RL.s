;/*****************************************************************************/
;/* SAM9RL.S: Startup file for Atmel AT91SAM9R/RL device series               */
;/*****************************************************************************/
...
PMC_UPLLEN      EQU     (1<<16)         ; UTMI PLL Enable
PMC_MUL         EQU     (0x7FF<<16)     ; PLL Multiplier
PMC_MOSCS       EQU     (1<<0)          ; Main Oscillator Stable
PMC_LOCK        EQU     (1<<1)          ; PLL Lock Status
PMC_LOCKU       EQU     (1<<6)          ; UPLL Lock Status
...
;//     <o2.11>     PID11: Two-Wire Interface 0 Enable
;//     <o2.12>     PID12: Two-Wire Interface 1 Enable
;//     <o2.13>     PID13: Serial Peripheral Interface Enable
;//     <o2.14>     PID14: Synchronous Serial Controller 0 Enable
;//     <o2.15>     PID15: Synchronous Serial Controller 1 Enable
;//     <o2.16>     PID16: Timer Counter 0 Enable
;//     <o2.17>     PID17: Timer Counter 1 Enable
;//     <o2.18>     PID18: Timer Counter 2 Enable
;//     <o2.19>     PID19: Pulse Width Modulation Controller Enable
;//     <o2.20>     PID20: Touch Screen Controller Enable
;//     <o2.21>     PID21: DMA Controller Enable
;//     <o2.22>     PID22: USB Device High Speed Enable
;//     <o2.23>     PID23: LCD Controller Enable
;//     <o2.24>     PID24: AC97 Controller Enable
;//     <o2.31>     PID31: Advanced Interrupt Controller (IRQ0) Enable
;//   </h>
;//
;//   <h> UTMI Clock Configuration Register (CKGR_UCKR)
;//     <o3.16>     UPLLEN: UTMI PLL Enable
;//     <o3.20..23> PLLCOUNT: UTMI PLL Startup Time <0-15>
;//     <o3.24>     BIASEN: UTMI BIAS Enable
;//     <o3.28..31> BIASCOUNT: UTMI BIAS Startup Time <0-15>
;//   </h>
;//
;//   <h> Main Oscillator Register (CKGR_MOR)
;//     <o4.0>      MOSCEN: Main Oscillator Enable
;//     <o4.8..15>  OSCOUNT: Main Oscillator Startup Time <0-255>
;//   </h>
;//
;//   <h> Clock Generator Phase Locked Loop Register (CKGR_PLLR)
;//                   <i> PLL Freq = (Main CLOCK Freq / DIV) * (MUL + 1)
;//                   <i> Example: XTAL = 12 MHz, DIV = 3, MUL = 50  =>  PLL = 200 MHz
;//     <o5.0..7>   DIV: PLL Divider <0-255>
;//                   <i> 0        - Divider output is 0
;//                   <i> 1        - Divider is bypassed
;//                   <i> 2 .. 255 - Divider output is the Main Clock divided by DIV
;//     <o5.8..13>  PLLCOUNT: PLL Counter <0-63>
;//                   <i> Number of Slow Clocks before the LOCK bit is set in
;//                   <i> PMC_SR after CKGR_PLLR is written
;//     <o5.14..15> OUT: PLL Clock Frequency Range
;//                   <0=> 80 .. 200MHz   <1=> Reserved
;//                   <2=> 190 .. 240MHz  <3=> Reserved
;//     <o5.16..26> MUL: PLL Multiplier <0-2047>
...
;//                   <0=> Master Clock = Processor Clock
;//                   <1=> Master Clock = Processor Clock / 2
;//                   <2=> Master Clock = Processor Clock / 4
;//                   <3=> Reserved
;//   </h>
;//
...
PMC_PCER_Val    EQU     0x00000000
CKGR_UCKR_Val   EQU     0x00000000
CKGR_MOR_Val    EQU     0x00000701
CKGR_PLLR_Val   EQU     0x2031BF03
PMC_MCKR_Val    EQU     0x00000102
...
MATRIX_TCR_OFS   EQU    0x114           ; TCM Configuration Reg   Address Offset
EBI_CSA_OFS      EQU    0x120           ; EBI Chip Select 0 Asign Address Offset

; Constants
EBI_CS0_ADDRESS  EQU    0x10000000      ; Start of EBI memory addressed by CS0
EBI_CS1_ADDRESS  EQU    0x20000000      ; Start of EBI memory addressed by CS1
EBI_CS2_ADDRESS  EQU    0x30000000      ; Start of EBI memory addressed by CS2
EBI_CS3_ADDRESS  EQU    0x40000000      ; Start of EBI memory addressed by CS3
EBI_CS4_ADDRESS  EQU    0x50000000      ; Start of EBI memory addressed by CS4
EBI_CS5_ADDRESS  EQU    0x60000000      ; Start of EBI memory addressed by CS5
...
; External Bus (EBI) User Interface
...
MOSCS_Loop      LDR     R2, [R0, #PMC_SR_OFS]
                ...
                ; Setup the PLL
                IF      (CKGR_PLLR_Val:AND:PMC_MUL) != 0  
                LDR     R1, =CKGR_PLLR_Val
                STR     R1, [R0, #CKGR_PLLR_OFS]

                ; Wait until PLL is stabilized
PLL_Loop        LDR     R2, [R0, #PMC_SR_OFS]
                ANDS    R2, R2, #PMC_LOCK
                BEQ     PLL_Loop
                ENDIF

                ; Setup the UPLL
                IF      (CKGR_UCKR_Val:AND:PMC_UPLLEN) != 0  
                LDR     R1, =CKGR_UCKR_Val
                STR     R1, [R0, #CKGR_UCKR_OFS]

                ; Wait until UPLL is stabilized
UPLL_Loop       LDR     R2, [R0, #PMC_SR_OFS]
                ANDS    R2, R2, #PMC_LOCKU
                BEQ     UPLL_Loop
                ENDIF
...
; Tightly-Coupled Memory Setup -------------------------------------------------

                IF      TCM_SETUP != 0  
                LDR     R0, =MATRIX_BASE        ; Setup TCR register
                MOV     R1, #ITCM_SIZE_Val
                ORR     R1, R1, #DTCM_SIZE_Val:SHL:4
                STR     R1, [R0, #MATRIX_TCR_OFS]

                MRC     p15, 0, R0, c9, c1, 0   ; Enable Data TCM
                ORR     R0, R0, #DTCM_SIZE_Val:SHL:2
                ORR     R0, R0, #DTCM_ADDR_Val:SHL:12
                ORR     R0, R0, #TCM_ENABLE
                MCR     p15, 0, R0, c9, c1, 0

                MRC     p15, 0, R0, c9, c1, 1   ; Enable Instruction TCM
                ORR     R0, R0, #ITCM_SIZE_Val:SHL:2
                ORR     R0, R0, #ITCM_ADDR_Val:SHL:12
                ORR     R0, R0, #TCM_ENABLE
