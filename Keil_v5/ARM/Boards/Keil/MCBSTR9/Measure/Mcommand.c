...
void measure_display (struct mrec display)  {
  ...
  printf ("\rTime: %2d:%02d:%02d.%03d GPIO3:%02X",
           ...
           (int)display.gpio);           /* display gpio port value           */

  for (i = 0; i < 4; i++)  {             /* display AD0 through AD3           */
    printf (" A%d:%4.2fV", i, (float) display.analog[i] * 3.3 / 0x3FF);
  }
}
...
void set_time (char * buffer)  {
  ...
  if (hour > 23  ||  min > 59  ||        /* check for valid inputs            */
    ...
    VIC0->INTECR |=  (0x01 << (TIM3_ITLine)); /* disable interrupts while     */
    ...
    interval.sec = 0;
    interval.msec = 0;
    VIC0->INTER  |=  (0x01 << (TIM3_ITLine)); /* enable interrupts again      */
  }
}
