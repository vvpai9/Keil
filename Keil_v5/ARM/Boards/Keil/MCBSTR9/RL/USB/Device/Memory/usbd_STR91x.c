 ...
 *----------------------------------------------------------------------------
 *      Name:    usbd_STR91x.c
 *      Purpose: Hardware Layer module for ST STR91x
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
static void USBD_ISR (void) __irq;
...
void USBD_Init (void) {
  ...
  VIC0->VAiR[0] =  (u32)USBD_ISR;       /* Setup ISR                          */
...
void USBD_Reset (void) {
  ...
  FreeBufAddr += USBD_MAX_PACKET0;
  if (USBD_MAX_PACKET0 > 60) {
    pBUF_DSCR->COUNT_TXRX = (((USBD_MAX_PACKET0 << 5) - 1) | 0x8000UL) << 16;
  } else {
    pBUF_DSCR->COUNT_TXRX =   (USBD_MAX_PACKET0 << 9)                  << 16;
  }
...
void USBD_ConfigEP (USB_ENDPOINT_DESCRIPTOR *pEPD) {
  ...
  if (pEPD->bEndpointAddress & USB_ENDPOINT_DIRECTION_MASK) {
    ...
    if (val > 60) {
      val = (val + 31) & ~31;
      (pBUF_DSCR + num)->COUNT_TXRX = (((val << 5) - 1) | 0x8000UL) << 16;
    } else {
      val = (val + 1)  & ~1;
      (pBUF_DSCR + num)->COUNT_TXRX =   (val << 9)                  << 16;
    }
  }
  FreeBufAddr += val;
