 ...
 *----------------------------------------------------------------------------
 *      Name:    CAN_STR71x.c
 *      Purpose: CAN Driver, Hardware specific module for ST STR71x
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
static CAN_ERROR CAN_hw_set_baudrate (U32 ctrl, U32 baudrate)  {
  ...
  /* Determine which nominal time to use for requestd baudrate and set
     appropriate bit timing                                                  */
  if (baudrate <= 500000)  {
    brp  = (RCCU_FrequencyValue(RCCU_FCLK) / 16) / baudrate;

    /* Load the baudrate registers BTR and BRPR registers                    */
    CAN_SetTiming(11, 4, 4, brp);
  }  else if (baudrate <= 1000000)  {
    brp  = (RCCU_FrequencyValue(RCCU_FCLK) / 8 ) / baudrate;
...
CAN_ERROR CAN_hw_setup (U32 ctrl)  {
  ...
  /* Setup RX pin as input standard TTL                                      */
  GPIO_Config   (GPIO1, 0x0800, GPIO_IN_TRI_TTL);

  /* Setup TX pin as output alternate push-pull                              */
  GPIO_Config   (GPIO1, 0x1000, GPIO_AF_PP);

  /* Setup STB pin as output push-pull, and clear it                         */
  GPIO_Config   (GPIO1, 0x0008, GPIO_OUT_PP);
  GPIO_BitWrite (GPIO1, 3, 0);

  /* Setup IRQ vector                                                        */
  EIC->IVR = (U32)CAN_IRQHandler;
  EIC->SIR[CAN_IRQChannel] = (((U32)CAN_IRQHandler) << 16);
...
CAN_ERROR CAN_hw_init (U32 ctrl, U32 baudrate)  {
  ...
  CAN_InvalidateAllMsgObj();          /* Initialize all objects as unused    */
...
static void CAN_hw_rd (U32 ctrl, U32 ch, CAN_msg *msg)  {
  ...
  if ((CAN->sMsgObj[1].A2R & CAN_A2R_XTD) == 0)  {  /* Standard ID           */
    ...
    msg->id     =   ((U32)(CAN->sMsgObj[1].A2R) >> 16)
                  | ((U32)(CAN->sMsgObj[1].A1R) & 0x1FFF);
  }
...
#define CTRL                  1
#define CANc                  CAN
#define CAN_INT_CLR           (1 << CAN_IRQChannel)
