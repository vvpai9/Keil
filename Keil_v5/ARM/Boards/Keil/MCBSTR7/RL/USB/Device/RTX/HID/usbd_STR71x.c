 ...
 *----------------------------------------------------------------------------
 *      Name:    usbd_STR71x.c
 *      Purpose: Hardware Layer module for ST STR71x
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
void USBD_Init (void) {
  ...
  EIC->IVR = (u32)USBD_ISR;
  EIC->SIR[USBLP_IRQChannel] = ((u32)USBD_ISR << 16);
  EIC_IRQChannelPriorityConfig(USBLP_IRQChannel, 1);
...
static void USBD_ISR (void) __irq {
  ...
  EIC->IPR = 1 << USBLP_IRQChannel;                     /* Acknowledge Interrupt */
}
