...
void Out_Val (void) {
  ...
  os_dly_wait (200);
  LCD_print (0, 0, "CAN1: Set= 0x00 ");
  LCD_print (0, 1, "CAN2: Rx = 0x00 ");

  for (;;) {
    sprintf ((char *)lcd_text[0],"%.2X", Tx_val);
    sprintf ((char *)lcd_text[1],"%.2X", Rx_val);
...
/*----------------------------------------------------------------------------
 *  Task 1: Sends REMOTE FRAME request on CAN 2, and display value received 
 *          on CAN 2
 *---------------------------------------------------------------------------*/
