...
BOOL usbh_ohci_hw_init (BOOL on) {
  ...
  if (on) {
    ...
    USBClkCtrl    |=  0x19;             /* Enable Host, OTG and AHB master clk*/

    for (tout = 100; ; tout--) {
      if ((USBClkSt & 0x19) == 0x19)    /* Wait for clocks enabled            */
        break;
      if (!tout) 
        return (__FALSE);
    }
    ...
    USBClkCtrl  &= ~0x19;               /* Disable Host,OTG and AHB master clk*/

    for (tout = 100; ; tout--) {
      if ((USBClkSt & 0x19) == 0)       /* Wait for clocks disabled           */
        break;
      if (!tout) 
        return (__FALSE);
    }
