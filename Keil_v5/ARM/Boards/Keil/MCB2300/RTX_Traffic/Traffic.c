...
#define red     0x000001              /* I/O Pin:  red    lamp output        */
#define yellow  0x000002              /* I/O Pin:  yellow lamp output        */
#define green   0x000004              /* I/O Pin:  green  lamp output        */
#define stop    0x000010              /* I/O Pin:  stop   lamp output        */
#define walk    0x000020              /* I/O Pin:  walk   lamp output        */
#define key     0x000400              /* I/O Pin:  self-service key input    */
#define SET(o,s) {        \
  if (s==0) FIO2CLR = o;  \
  else      FIO2SET = o;  \
};
...
__task void keyread (void) {
  while (1) {                                 /* endless loop                */
    if ((FIO2PIN & key) == 0) {               /* if key pressed              */
...
/*----------------------------------------------------------------------------
 *        Task 7 'lcd': LCD Control task
 *---------------------------------------------------------------------------*/
__task void lcd (void) {
  U32 i;

  LCD_init ();                                /* Init LCD display module     */
  LCD_cur_off ();
  for (;;) {
    LCD_cls ();
    LCD_puts ("    RL-ARM      "
