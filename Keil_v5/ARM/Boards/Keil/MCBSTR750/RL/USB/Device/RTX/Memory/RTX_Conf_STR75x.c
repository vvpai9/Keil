 ...
 *----------------------------------------------------------------------------
 *      Name:    RTX_CONFIG.C
 *      Purpose: Configuration of RTX Kernel for ST STR75x
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
//   <i> Default: 60000000  (60MHz TIM_ICK)
#ifndef OS_CLOCK
...
#define OS_TIM_         (5 << OS_TID_)                  /*  Interrupt Mask  */
#define OS_TRV          ((U16)(((double)OS_CLOCK*(double)OS_TICK)/1E8))
#define OS_TVAL         ((TIMx->CNT) & 0xFFFF)          /*  Timer Value     */
#define OS_TOVF          (TIMx->ISR & TIM_IT_Update)    /*  Overflow Flag   */
#define OS_TFIRQ()      TIMx->RER  |= TIM_IT_OC1;       /*  Force Interrupt */
#define OS_TIACK()      TIMx->RER  &= ~TIM_IT_OC1;      /*  Interrupt Ack   */ \
                        TIMx->ISR   = ~TIM_IT_Update;                          \
                        EIC->IPR    =  OS_TIM_;
#define OS_TINIT()      TIMx->ARR   =  OS_TRV;          /*  Initialization  */ \
                        TIMx->PSC   =  (100 - 1);                              \
                        TIMx->OCR1  =  0xFFFC;                                 \
                        TIMx->RSR   =  TIM_IT_OC1 | TIM_IT_Update;             \
                        TIMx->RER   =  TIM_IT_Update;                          \
                        TIMx->CR    =  0x0006;                                 \
                        EIC->IVR    =  (U32)os_clock_interrupt;                \
                        EIC->SIRn[OS_TID_]   =                                 \
                        EIC->SIRn[OS_TID_+2] = 1 |                             \
