...
ICLK0_CLK_ST_OFS EQU    0x8C            ; BASE_ICLK0_CLK Config Reg Offset  
ICLK0_CLK_CO_OFS EQU    0x90            ; BASE_ICLK0_CLK Config Reg Offset  
...
ICLK1_CLK_ST_OFS  EQU   0xBC            ; BASE_ICLK1_CLK Config Reg Offset  
ICLK1_CLK_CO_OFS  EQU   0xC0            ; BASE_ICLK1_CLK Config Reg Offset  

; Clock Generation Unit 1(CGU1) Module Definitions
CGU1_BASE           EQU     0xFFFFB000  ; CGU1 Base Address
FREQ1_MON_OFS       EQU     0x14        ; Frequency Monitor Register Offset
RDET1_OFS           EQU     0x18        ; Clock Detection Register Offset
PLL1_STATUS_OFS     EQU     0x1C        ; PLL Status Register Offset
PLL1_CONTROL_OFS    EQU     0x20        ; PLL Control Register Offset
FDIV1_STAT0_OFS     EQU     0x24        ; FDIV0 Status Register Offset
FDIV1_CTRL0_OFS     EQU     0x28        ; FDIV0 Control Register Offset
USB_CLK_ST_OFS      EQU     0x2C        ; BASE_USB_CLK Status Register Offset
USB_CLK_CO_OFS      EQU     0x30        ; BASE_USB_CLK Config Register Offset
USB_I2C_CLK_ST_OFS  EQU     0x2C        ; BASE_USB_I2C_CLK Status Register Offset
USB_I2C_CLK_CO_OFS  EQU     0x30        ; BASE_USB_I2C_CLK Config Register Offset
OUT_CLK_ST_OFS      EQU     0x34        ; BASE_OUT_CLK Status Register Offset
OUT_CLK_CO_OFS      EQU     0x38        ; BASE_OUT_CLK Config Register Offset
...
;//   </h>
FDIV_CTRL0_Val  EQU     0x02001030
FDIV_CTRL1_Val  EQU     0x02001006
FDIV_CTRL2_Val  EQU     0x020011E0
FDIV_CTRL3_Val  EQU     0x00001001
FDIV_CTRL4_Val  EQU     0x00001001
FDIV_CTRL5_Val  EQU     0x00001001
FDIV_CTRL6_Val  EQU     0x02001001
...
;//   <h> Output-clock Configuration Register for BASE_ICLK0_CLK (ICLK0_CLK_CONF)
;//     <o0.24..31> CLK_SEL: Clock Source Selection
;//                     <0=> LP_OSC
;//                     <1=> Crystal Oscillator
;//                     <2=> PLL
...
;//     <o0.0>           PD: Power-down Clock Slice
;//   </h>
ICLK0_CLK_Val     EQU     0x0200000C
;//   <h> Output-clock Configuration Register for BASE_ICLK1_CLK (ICLK1_CLK_CONF)
;//     <o0.24..31> CLK_SEL: Clock Source Selection
;//                     <0=> LP_OSC
;//                     <1=> Crystal Oscillator
;//                     <2=> PLL
...
;//     <o0.0>           PD: Power-down Enable
;//   </h>
PLL1_CONTROL_Val EQU     0x01000004
...
;//     <o0.0..11> DENOMINATOR: Denominator or Modulo Value <0-4095>
;//   </h>
FDIV1_CTRL0_Val  EQU     0x02001028
;// </e> CGU1 Setup
;// </e> Clock Setup
...
;//     </h>
;//   </e>
SMC_BANK1_SETUP     EQU 1
SMBIDCYR1_Val       EQU 0x0000000A
SMBWST1R1_Val       EQU 0x0000000A
SMBWST2R1_Val       EQU 0x00000005
SMBWSTOENR1_Val     EQU 0x00000000
SMBWSTWENR1_Val     EQU 0x00000001
SMBCR1_Val          EQU 0x00000041
...
IRQ_Table       DCD     0                    ;0.  ; Vector 0
                ...
                DCD     ADC0_Handler         ;17. ; ADC 0 IRQ Handler                 LPC292X
                DCD     1                         ; ADC 0 IRQ Priority
                ...
                DCD     0                    ;45. ; Reserved
                DCD     0                         ; Reserved
                DCD     USB_IRQ_H_Handler    ;46. ; USB Device IRQ High             LPC292X
                DCD     1                         ; USB Device IRQ High Priority     
                DCD     USB_IRQHandler       ;47. ; USB Device IRQ Low              LPC292X
                DCD     1                         ; USB Device IRQ Low Priority
                PRESERVE8
...
FDIV6_Locking   LDR     R2, [R0, #RDET_OFS]
                ...
                IF      ICLK0_CLK_Val != 0
                LDR     R1, =ICLK0_CLK_Val    ; Setup ICLK0 Clock Config
                STR     R1, [R0, #ICLK0_CLK_CO_OFS] 
                ENDIF

                IF      ICLK1_CLK_Val != 0
                LDR     R1, =ICLK1_CLK_Val    ; Setup ICLK1 Clock Config
                STR     R1, [R0, #ICLK1_CLK_CO_OFS] 
                ENDIF
...
;  If PLL is used
                IF      (PLL1_CONTROL_Val:AND:PLL_PD) = 0
                LDR     R1, =((PLL1_CONTROL_Val:AND:P23EN):OR:PLL_PD)
                STR     R1, [R0, #PLL1_CONTROL_OFS]
                LDR     R1, =(PLL1_CONTROL_Val:OR:PLL_PD)
                STR     R1, [R0, #PLL1_CONTROL_OFS]
                LDR     R1, =PLL1_CONTROL_Val
                STR     R1, [R0, #PLL1_CONTROL_OFS]

;  Wait until PLL Locked
PLL1_Not_Locked LDR     R2, [R0, #PLL1_STATUS_OFS]
                ANDS    R2, R2, #PLL_LOCK
                BEQ     PLL1_Not_Locked
                ENDIF

;  Wait until PLL Clock Present
PLL1_Not_Present LDR     R2, [R0, #RDET_OFS]
                 ANDS    R2, R2, #PLL_PRESENT
                 BEQ     PLL1_Not_Present

                IF      FDIV1_CTRL0_Val != 0x00001001
                LDR     R1, =FDIV1_CTRL0_Val
                STR     R1, [R0, #FDIV1_CTRL0_OFS]
FDIV1_0_Locking LDR     R2, [R0, #RDET_OFS]
                ANDS    R2, R2, #FDIV0_PRESENT
                BEQ     FDIV1_0_Locking
                ENDIF
