...
extern BOOL LEDrun;
extern void LED_out (U32 mask, U32 val);
  ...
  switch (code) {
   ...
   do {
      ...
      if (var[0] != 0) {
         ...
         else if (str_scomp (var, "ctrl=Board") == __TRUE) {
            LEDrun = __FALSE;
            LEDblink = __TRUE;
         }
   ...
   free_mem ((OS_FRAME *)var);
   P2 = (LEDblink) ? 0 : P2;         // only if not under Board control
   LED_out (0x0F, P2);
...
U16 cgi_func (U8 *env, U8 *buf, U16 buflen, U32 *pcgi) {
   ...
   switch (env[0]) {
      ...
      case 'b':
         ...
         if (env[2] == 'c') {
            ...
            len = sprintf((char *)buf,(const char *)&env[4],
                    ((LEDrun == __FALSE) && (LEDblink == __FALSE)) ? "selected" : "",   // Browser is selected
                    ((LEDrun == __TRUE)  && (LEDblink == __FALSE)) ? "selected" : "",   // Running is selected
                    ((LEDrun == __FALSE) && (LEDblink == __TRUE))  ? "selected" : "");  // Board   is selected
            break;
         }
         /* LED CheckBoxes */
         id = env[2] - '0';
      ...
      case 'm':         
         /* MEMS Module control - file 'mems.cgi' */
         switch (env[2]) {
            case '1':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outx_h);
               break;
            case '2':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outx_l);
               break;
            case '3':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outy_h);
               break;
            case '4':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outy_l);
               break;
            case '5':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outz_h);
               break;
            case '6':
               len = sprintf((char *)buf,(const char *)&env[4],mems_data.outz_l);
               break;
            case 'x':
               c = mems_data.outx_h;
               if (c > 100) c = 50+(256-c)*8;
                  else c = 50-c*8;
               len = sprintf((char *)buf,(const char *)&env[4],c);
               break;
            case 'y':
               c = mems_data.outy_h;
               if (c > 100) c = 50+(256-c)*8;
