...
// =============================
//   <o>Hardware timer <0=> High Speed Timer
 ...
 *---------------------------------------------------------------------------*/
 ...
 #define OS_TOVF        (HSTIM_INT & (1 << 2))          /*  Overflow Flag   */
 #define OS_TFIRQ()     HSTIM_MATCH1  =  HSTIM_COUNTER+1; /* Force Interrupt*/
 #define OS_TIACK()     HSTIM_INT     =  0x06;          /*  Interrupt Ack   */ \
                        MIC_RSR       =  OS_TIM_;                              \
                        HSTIM_MATCH1  =  OS_TRV + 1;
...
extern void interrupt_mstim    (void) __irq;
...
__asm void IRQ_Handler_RTX (void) {
   /* Common IRQ Handler */
        ...
        TST     R0,#(1 << 27)                  ; Check Millisecond Timer Flag
        LDMNEIA SP!,{R0}                       ; Restore R0
        LDRNE   PC,=__cpp(interrupt_mstim)
