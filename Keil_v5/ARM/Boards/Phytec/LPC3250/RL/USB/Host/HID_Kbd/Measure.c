...
const char menu[] =
   ...
   "| Recorder. It is based on the LPC325x |\n"
   "| and records the state of the voltage |\n"
   "| on the analog input ADIN2 .          |\n"
   "+ command ----+ function --------------+\n"
   "| R [n]       | read <n> records       |\n"
...
int ADC_GetConversion (void) {
  ...
  return (ADDAT & 0x03FF);                     /* AD value (10 bit)           */
}
...
int main (void)  {                             /* main entry for program      */
  ...
  /* High Speed Timer as interval timer, reload to 1ms.                       */
  TIMCLK_CTRL   = 0x02;
  HSTIM_MATCH2  = 1000 - 1;                    /* 1 mSec                      */
