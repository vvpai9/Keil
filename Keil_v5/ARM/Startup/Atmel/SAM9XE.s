;/*****************************************************************************/
;/* SAM9XE.S: Startup file for Atmel AT91SAM9XE device series                 */
;/*****************************************************************************/
...
;----------------------- Enhanced Embedded Flash Controller Definitions --------

; Enhanced Embedded Flash Controller (EEFC) definitions
EEFC_FMR_Adr    EQU     0xFFFFFA00      ; Flash Mode Register     Address

;// <e> Enhanced Embedded Flash Controller (EEFC)
;//   <h> Flash Mode Register Register (EEFC_FMR)
;//     <o1.8..11>  FWS: Flash Wait State <1-16><#-1>
;//       <i> Number of cycles for Read/Write operations
;//     <o1.0>      FRDY: Ready Interrupt Enable
;//   </h>
;// </e>
EEFC_SETUP      EQU     1
EEFC_FMR_Val    EQU     0x00000200
...
;//     <o2.14>     PID14: Serial Synchronous Controller Enable
...
;//     <o2.25>     PID25: Two-Wire Interface 1 Enable
...
;//     <o0.3>        Enable A24 Pin
;//     <o0.4>        Enable A25 Pin
;//     <o0.10>       Enable NCS2 Pin
;//     <o0.11>       Enable NCS3 Pin
;//     <o0.12>       Enable NCS4 Pin
;//     <o0.13>       Enable NCS5 Pin
;//     <o0.14>       Enable NCS6 Pin
;//     <o0.15>       Enable NCS7 Pin
;//   </e0.0> Setup Pin Configuration
SMC_PIN_SETUP   EQU     0x00000000
...
;//     <o0.4>      Enable A25 Pin
;//   </e0.0> Setup Pin Configuration
SDRAM_PIN_SETUP EQU     0x00000003
...
; Setup Enhanced Embedded Flash Controller -------------------------------------
...
; Setup Static Memory Controller (SMC) -----------------------------------------
                ...
                ; Setup pins
                IF      ((SMC_PIN_SETUP:AND:(1 <<  0)) != 0)
                LDR     R0, =PIOC_BASE
                MOV     R1, #0
                MOV     R2, #0
                IF      ((SMC_PIN_SETUP:AND:(1 <<  1)) != 0)
                LDR     R1, =0xFFFF0000  
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 <<  2)) != 0)
                ORR     R1, #(1 <<  4)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 <<  3)) != 0)
                ORR     R1, #(1 <<  5)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 <<  4)) != 0)
                ORR     R1, #(1 << 10)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 10)) != 0)
                ORR     R1, #(1 << 11)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 11)) != 0)
                ORR     R1, #(1 << 14)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 12)) != 0)
                ORR     R1, #(1 <<  8)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 13)) != 0)
                ORR     R1, #(1 <<  9)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 14)) != 0)
                ORR     R2, #(1 << 13)
                ENDIF
                IF      ((SMC_PIN_SETUP:AND:(1 << 15)) != 0)
                ORR     R2, #(1 << 12)
                ENDIF

                STR     R1, [R0, #PIO_PUDR_OFS]
                STR     R2, [R0, #PIO_PUDR_OFS]
                STR     R1, [R0, #PIO_ASR_OFS]
                STR     R2, [R0, #PIO_BSR_OFS]
                STR     R1, [R0, #PIO_PDR_OFS]
                STR     R2, [R0, #PIO_PDR_OFS]
                ENDIF   ; of IF      ((SMC_PIN_SETUP:AND:(1 <<  0)) != 0)
...
; Setup SDRAM Controller (SDRAMC) ----------------------------------------------
                ...
                ; Setup pins
                IF      ((SDRAM_PIN_SETUP:AND:(1 <<  0)) != 0)
                LDR     R0, =PIOC_BASE
                MOV     R1, #0
                IF      ((SDRAM_PIN_SETUP:AND:(1 <<  1)) != 0)
                LDR     R1, =0xFFFF0000  
                ENDIF
                IF      ((SDRAM_PIN_SETUP:AND:(1 <<  2)) != 0)
                ORR     R1, #(1 <<  4)
                ENDIF
                IF      ((SDRAM_PIN_SETUP:AND:(1 <<  3)) != 0)
                ORR     R1, #(1 <<  5)
                ENDIF
                IF      ((SDRAM_PIN_SETUP:AND:(1 <<  4)) != 0)
                ORR     R1, #(1 << 10)
                ENDIF
                ...
                STR     R1, [R0, #PIO_ASR_OFS]
                STR     R1, [R0, #PIO_PDR_OFS]
                ENDIF   ; of IF      ((SDRAM_PIN_SETUP:AND:(1 <<  0)) != 0)
