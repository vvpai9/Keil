...
#define NET_IP_LHOST        5           ///< IP localhost module
...
typedef struct arp_info {
  ...
  uint8_t IpAdr[IP_ADDR_LEN];           ///< Ethernet IP Address
} ARP_INFO;
...
typedef struct igmp_info {
  ...
  uint8_t GrpIpAdr[IP_ADDR_LEN];        ///< Group IP address
} IGMP_INFO;
...
typedef struct tcp_info {
  ...
  uint8_t  RemIpAdr[IP_ADDR_LEN];       ///< Remote IP address
...
typedef struct bsd_info {
  ...
  uint8_t  Event;                       ///< Wait Event flags in RTOS environment
...
typedef struct ftp_info {
  ...
  uint8_t  RemIpAdr[IP_ADDR_LEN];       ///< Client IP address
...
typedef struct com_dev {
  ARM_DRIVER_UART *DrvUart;             ///< Registered UART driver
...
typedef struct bsd_cfg {
  ...
  bool     InRtx;                       ///< Running in RTX environment
...
typedef struct snmp_cfg {
  ...
  uint8_t  TrapIp[IP_ADDR_LEN];         ///< Trap Server IP Address
...
typedef struct sntp_cfg {
  uint8_t  SrvIp[IP_ADDR_LEN];          ///< NTP/SNTP Server IP Address
...
extern bool eth_chk_adr (NET_FRAME *frame);

/// \brief Get MAC address for the given IP address.
/// \param[in]     ipadr         IP address of a host.
/// \return        Pointer to the MAC address in ARP cache.
extern const uint8_t *eth_get_adr (const uint8_t *ipadr);
