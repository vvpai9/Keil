...
#if (PPP_ENABLE)
  /* PPP device UART and MODEM drivers */
  extern ARM_DRIVER_UART CREATE_SYMBOL (Driver_UART, PPP_UART_DRIVER);
  extern DRIVER_MODEM    Driver_MODEM;

  /* PPP device configuration */
  PPP_CFG ppp_config = {
    /* Modem device */
    { &CREATE_SYMBOL (Driver_UART, PPP_UART_DRIVER),
      &Driver_MODEM,
...
#if (SLIP_ENABLE)
  /* SLIP device UART and MODEM drivers */
  extern ARM_DRIVER_UART CREATE_SYMBOL (Driver_UART, SLIP_UART_DRIVER);
  extern DRIVER_MODEM    Driver_MODEM;

  /* SLIP device configuration */
  SLIP_CFG slip_config = {
    /* Modem device */
    { &CREATE_SYMBOL (Driver_UART, SLIP_UART_DRIVER),
      &Driver_MODEM,
...
bool eth_chk_adr (NET_FRAME *frame)                     { return (false); }
const uint8_t *eth_get_adr (const uint8_t *ipadr)       { return (NULL);  }
