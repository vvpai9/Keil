...
static ARM_ETH_STATUS SetInterface (ARM_ETH_INTERFACE interface) {
  ...
  return (reg_wr(ETH_PHY_ADDR, REG_XCNTL, val));
}
...
static ARM_ETH_LINK_INFO GetLinkInfo (void) {
  ...
  reg_rd(ETH_PHY_ADDR, REG_XSTAT, &val);
  info.speed  = (val & XSTAT_SPEED)  ? ARM_ETH_SPEED_100M  : ARM_ETH_SPEED_10M;
  info.duplex = (val & XSTAT_DUPLEX) ? ARM_ETH_DUPLEX_FULL : ARM_ETH_DUPLEX_HALF;
